// ==============================================================
// File generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2015.4
// Copyright (C) 2015 Xilinx Inc. All rights reserved.
// ==============================================================

#include <systemc>
#include <iostream>
#include <cstdlib>
#include <cstddef>
#include <stdint.h>
#include "SysCFileHandler.h"
#include "ap_int.h"
#include "ap_fixed.h"
#include <complex>
#include <stdbool.h>
#include "autopilot_cbe.h"
#include "ap_stream.h"
#include "hls_stream.h"
#include "hls_half.h"

using namespace std;
using namespace sc_core;
using namespace sc_dt;


// [dump_struct_tree [build_nameSpaceTree] dumpedStructList] ---------->


// [dump_enumeration [get_enumeration_list]] ---------->


// wrapc file define: "gmem"
#define AUTOTB_TVIN_gmem  "../tv/cdatafile/c.bgsub.autotvin_gmem.dat"
#define AUTOTB_TVOUT_gmem  "../tv/cdatafile/c.bgsub.autotvout_gmem.dat"
// wrapc file define: "gmem_offset"
#define AUTOTB_TVIN_gmem_offset  "../tv/cdatafile/c.bgsub.autotvin_gmem_offset.dat"
#define AUTOTB_TVOUT_gmem_offset  "../tv/cdatafile/c.bgsub.autotvout_gmem_offset.dat"
// wrapc file define: "frame_in"
#define AUTOTB_TVIN_frame_in  "../tv/cdatafile/c.bgsub.autotvin_frame_in.dat"
// wrapc file define: "frame_out"
#define AUTOTB_TVIN_frame_out  "../tv/cdatafile/c.bgsub.autotvin_frame_out.dat"
// wrapc file define: "init"
#define AUTOTB_TVIN_init  "../tv/cdatafile/c.bgsub.autotvin_init.dat"
// wrapc file define: "bgmodel"
#define AUTOTB_TVIN_bgmodel  "../tv/cdatafile/c.bgsub.autotvin_bgmodel.dat"

#define INTER_TCL  "../tv/cdatafile/ref.tcl"

// tvout file define: "gmem"
#define AUTOTB_TVOUT_PC_gmem  "../tv/rtldatafile/rtl.bgsub.autotvout_gmem.dat"
// tvout file define: "gmem_offset"
#define AUTOTB_TVOUT_PC_gmem_offset  "../tv/rtldatafile/rtl.bgsub.autotvout_gmem_offset.dat"

class INTER_TCL_FILE {
	public:
		INTER_TCL_FILE(const char* name) {
			mName = name;
			gmem_depth = 0;
			gmem_offset_depth = 0;
			frame_in_depth = 0;
			frame_out_depth = 0;
			init_depth = 0;
			bgmodel_depth = 0;
			trans_num =0;
		}

		~INTER_TCL_FILE() {
			mFile.open(mName);
			if (!mFile.good()) {
				cout << "Failed to open file ref.tcl" << endl;
				exit (1);
			}
			string total_list = get_depth_list();
			mFile << "set depth_list {\n";
			mFile << total_list;
			mFile << "}\n";
			mFile << "set trans_num "<<trans_num<<endl;
			mFile.close();
		}

		string get_depth_list () {
			stringstream total_list;
			total_list << "{gmem " << gmem_depth << "}\n";
			total_list << "{gmem_offset " << gmem_offset_depth << "}\n";
			total_list << "{frame_in " << frame_in_depth << "}\n";
			total_list << "{frame_out " << frame_out_depth << "}\n";
			total_list << "{init " << init_depth << "}\n";
			total_list << "{bgmodel " << bgmodel_depth << "}\n";
			return total_list.str();
		}

		void set_num (int num , int* class_num) {
			(*class_num) = (*class_num) > num ? (*class_num) : num;
		}
	public:
		int gmem_depth;
		int gmem_offset_depth;
		int frame_in_depth;
		int frame_out_depth;
		int init_depth;
		int bgmodel_depth;
		int trans_num;

	private:
		ofstream mFile;
		const char* mName;
};


#define bgsub AESL_ORIG_DUT_bgsub
extern void bgsub (
unsigned char frame_in[76800],
unsigned char frame_out[76800],
bool init,
float bgmodel[614400]);
#undef bgsub

void bgsub (
unsigned char frame_in[76800],
unsigned char frame_out[76800],
bool init,
float bgmodel[614400])
{
	fstream wrapc_switch_file_token;
	wrapc_switch_file_token.open(".hls_cosim_wrapc_switch.log");
	int AESL_i;
	if (wrapc_switch_file_token.good())
	{
		static unsigned AESL_transaction_pc = 0;
		string AESL_token;
		string AESL_num;
		static AESL_FILE_HANDLER aesl_fh;


		// output port post check: "gmem"
		aesl_fh.read(AUTOTB_TVOUT_PC_gmem, AESL_token); // [[transaction]]
		if (AESL_token != "[[transaction]]")
		{
			exit(1);
		}
		aesl_fh.read(AUTOTB_TVOUT_PC_gmem, AESL_num); // transaction number

		if (atoi(AESL_num.c_str()) == AESL_transaction_pc)
		{
			aesl_fh.read(AUTOTB_TVOUT_PC_gmem, AESL_token); // data

			sc_bv<8> *gmem_pc_buffer = new sc_bv<8>[153600];
			int i = 0;

			while (AESL_token != "[[/transaction]]")
			{
				bool no_x = false;
				bool err = false;

				// search and replace 'X' with "0" from the 1st char of token
				while (!no_x)
				{
					size_t x_found = AESL_token.find('X');
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'gmem', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
						AESL_token.replace(x_found, 1, "0");
					}
					else
					{
						no_x = true;
					}
				}

				no_x = false;

				// search and replace 'x' with "0" from the 3rd char of token
				while (!no_x)
				{
					size_t x_found = AESL_token.find('x', 2);

					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'gmem', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
						AESL_token.replace(x_found, 1, "0");
					}
					else
					{
						no_x = true;
					}
				}

				// push token into output port buffer
				if (AESL_token != "")
				{
					gmem_pc_buffer[i] = AESL_token.c_str();
					i++;
				}

				aesl_fh.read(AUTOTB_TVOUT_PC_gmem, AESL_token); // data or [[/transaction]]

				if (AESL_token == "[[[/runtime]]]" || aesl_fh.eof(AUTOTB_TVOUT_PC_gmem))
				{
					exit(1);
				}
			}

			// ***********************************
			if (i > 0)
			{
				// RTL Name: gmem
				{
					// bitslice(7, 0)
					// {
						// celement: frame_in(7, 0)
						// {
							sc_lv<8>* frame_in_lv0_0_76799_1 = new sc_lv<8>[153600];
						// }
						// celement: frame_out(7, 0)
						// {
							sc_lv<8>* frame_out_lv0_0_76799_1 = new sc_lv<8>[153600];
						// }
					// }

					// bitslice(7, 0)
					{
						int hls_map_index = 0;
						// celement: frame_in(7, 0)
						{
							// carray: (0) => (76799) @ (1)
							for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
							{
								hls_map_index++;
							}
						}
						// celement: frame_out(7, 0)
						{
							// carray: (0) => (76799) @ (1)
							for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
							{
								if (&(frame_out[0]) != NULL) // check the null address if the c port is array or others
								{
									frame_out_lv0_0_76799_1[hls_map_index++].range(7, 0) = sc_bv<8>(gmem_pc_buffer[hls_map_index].range(7, 0));
								}
							}
						}
					}

					// bitslice(7, 0)
					{
						int hls_map_index = 0;
						// celement: frame_in(7, 0)
						{
							// carray: (0) => (76799) @ (1)
							for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
							{
								// sub                    : i_0
								// ori_name               : frame_in[i_0]
								// sub_1st_elem           : 0
								// ori_name_1st_elem      : frame_in[0]
								// output_left_conversion : frame_in[i_0]
								// output_type_conversion : (frame_in_lv0_0_76799_1[hls_map_index++]).to_uint64()
								hls_map_index++;
							}
						}
						// celement: frame_out(7, 0)
						{
							// carray: (0) => (76799) @ (1)
							for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
							{
								// sub                    : i_0
								// ori_name               : frame_out[i_0]
								// sub_1st_elem           : 0
								// ori_name_1st_elem      : frame_out[0]
								// output_left_conversion : frame_out[i_0]
								// output_type_conversion : (frame_out_lv0_0_76799_1[hls_map_index++]).to_uint64()
								if (&(frame_out[0]) != NULL) // check the null address if the c port is array or others
								{
									frame_out[i_0] = (frame_out_lv0_0_76799_1[hls_map_index++]).to_uint64();
								}
							}
						}
					}
				}
			}

			// release memory allocation
			delete [] gmem_pc_buffer;
		}

		// output port post check: "gmem_offset"
		aesl_fh.read(AUTOTB_TVOUT_PC_gmem_offset, AESL_token); // [[transaction]]
		if (AESL_token != "[[transaction]]")
		{
			exit(1);
		}
		aesl_fh.read(AUTOTB_TVOUT_PC_gmem_offset, AESL_num); // transaction number

		if (atoi(AESL_num.c_str()) == AESL_transaction_pc)
		{
			aesl_fh.read(AUTOTB_TVOUT_PC_gmem_offset, AESL_token); // data

			sc_bv<32> *gmem_offset_pc_buffer = new sc_bv<32>[614400];
			int i = 0;

			while (AESL_token != "[[/transaction]]")
			{
				bool no_x = false;
				bool err = false;

				// search and replace 'X' with "0" from the 1st char of token
				while (!no_x)
				{
					size_t x_found = AESL_token.find('X');
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'gmem_offset', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
						AESL_token.replace(x_found, 1, "0");
					}
					else
					{
						no_x = true;
					}
				}

				no_x = false;

				// search and replace 'x' with "0" from the 3rd char of token
				while (!no_x)
				{
					size_t x_found = AESL_token.find('x', 2);

					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'gmem_offset', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
						AESL_token.replace(x_found, 1, "0");
					}
					else
					{
						no_x = true;
					}
				}

				// push token into output port buffer
				if (AESL_token != "")
				{
					gmem_offset_pc_buffer[i] = AESL_token.c_str();
					i++;
				}

				aesl_fh.read(AUTOTB_TVOUT_PC_gmem_offset, AESL_token); // data or [[/transaction]]

				if (AESL_token == "[[[/runtime]]]" || aesl_fh.eof(AUTOTB_TVOUT_PC_gmem_offset))
				{
					exit(1);
				}
			}

			// ***********************************
			if (i > 0)
			{
				// RTL Name: gmem_offset
				{
					// bitslice(31, 0)
					// {
						// celement: bgmodel(31, 0)
						// {
							sc_lv<32>* bgmodel_lv0_0_614399_1 = new sc_lv<32>[614400];
						// }
					// }

					// bitslice(31, 0)
					{
						int hls_map_index = 0;
						// celement: bgmodel(31, 0)
						{
							// carray: (0) => (614399) @ (1)
							for (int i_0 = 0; i_0 <= 614399; i_0 += 1)
							{
								if (&(bgmodel[0]) != NULL) // check the null address if the c port is array or others
								{
									bgmodel_lv0_0_614399_1[hls_map_index++].range(31, 0) = sc_bv<32>(gmem_offset_pc_buffer[hls_map_index].range(31, 0));
								}
							}
						}
					}

					// bitslice(31, 0)
					{
						int hls_map_index = 0;
						// celement: bgmodel(31, 0)
						{
							// carray: (0) => (614399) @ (1)
							for (int i_0 = 0; i_0 <= 614399; i_0 += 1)
							{
								// sub                    : i_0
								// ori_name               : bgmodel[i_0]
								// sub_1st_elem           : 0
								// ori_name_1st_elem      : bgmodel[0]
								// output_left_conversion : *(int*)&bgmodel[i_0]
								// output_type_conversion : (bgmodel_lv0_0_614399_1[hls_map_index++]).to_uint64()
								if (&(bgmodel[0]) != NULL) // check the null address if the c port is array or others
								{
									*(int*)&bgmodel[i_0] = (bgmodel_lv0_0_614399_1[hls_map_index++]).to_uint64();
								}
							}
						}
					}
				}
			}

			// release memory allocation
			delete [] gmem_offset_pc_buffer;
		}

		AESL_transaction_pc++;
	}
	else
	{
		static unsigned AESL_transaction;

		static AESL_FILE_HANDLER aesl_fh;

		// "gmem"
		char* tvin_gmem = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_gmem);
		char* tvout_gmem = new char[50];
		aesl_fh.touch(AUTOTB_TVOUT_gmem);

		// "gmem_offset"
		char* tvin_gmem_offset = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_gmem_offset);
		char* tvout_gmem_offset = new char[50];
		aesl_fh.touch(AUTOTB_TVOUT_gmem_offset);

		// "frame_in"
		char* tvin_frame_in = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_frame_in);

		// "frame_out"
		char* tvin_frame_out = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_frame_out);

		// "init"
		char* tvin_init = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_init);

		// "bgmodel"
		char* tvin_bgmodel = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_bgmodel);

		static INTER_TCL_FILE tcl_file(INTER_TCL);
		int leading_zero;

		// [[transaction]]
		sprintf(tvin_gmem, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_gmem, tvin_gmem);

		sc_bv<8>* gmem_tvin_wrapc_buffer = new sc_bv<8>[153600];

		// RTL Name: gmem
		{
			// bitslice(7, 0)
			{
				int hls_map_index = 0;
				// celement: frame_in(7, 0)
				{
					// carray: (0) => (76799) @ (1)
					for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : frame_in[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : frame_in[0]
						// regulate_c_name       : frame_in
						// input_type_conversion : frame_in[i_0]
						if (&(frame_in[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<8> frame_in_tmp_mem;
							frame_in_tmp_mem = frame_in[i_0];
							gmem_tvin_wrapc_buffer[hls_map_index++].range(7, 0) = frame_in_tmp_mem.range(7, 0);
						}
					}
				}
				// celement: frame_out(7, 0)
				{
					// carray: (0) => (76799) @ (1)
					for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : frame_out[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : frame_out[0]
						// regulate_c_name       : frame_out
						// input_type_conversion : frame_out[i_0]
						if (&(frame_out[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<8> frame_out_tmp_mem;
							frame_out_tmp_mem = frame_out[i_0];
							gmem_tvin_wrapc_buffer[hls_map_index++].range(7, 0) = frame_out_tmp_mem.range(7, 0);
						}
					}
				}
			}
		}

		// dump tv to file
		for (int i = 0; i < 153600; i++)
		{
			sprintf(tvin_gmem, "%s\n", (gmem_tvin_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_gmem, tvin_gmem);
		}

		tcl_file.set_num(153600, &tcl_file.gmem_depth);
		sprintf(tvin_gmem, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_gmem, tvin_gmem);

		// release memory allocation
		delete [] gmem_tvin_wrapc_buffer;

		// [[transaction]]
		sprintf(tvin_gmem_offset, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_gmem_offset, tvin_gmem_offset);

		sc_bv<32>* gmem_offset_tvin_wrapc_buffer = new sc_bv<32>[614400];

		// RTL Name: gmem_offset
		{
			// bitslice(31, 0)
			{
				int hls_map_index = 0;
				// celement: bgmodel(31, 0)
				{
					// carray: (0) => (614399) @ (1)
					for (int i_0 = 0; i_0 <= 614399; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : bgmodel[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : bgmodel[0]
						// regulate_c_name       : bgmodel
						// input_type_conversion : *(int*)&bgmodel[i_0]
						if (&(bgmodel[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> bgmodel_tmp_mem;
							bgmodel_tmp_mem = *(int*)&bgmodel[i_0];
							gmem_offset_tvin_wrapc_buffer[hls_map_index++].range(31, 0) = bgmodel_tmp_mem.range(31, 0);
						}
					}
				}
			}
		}

		// dump tv to file
		for (int i = 0; i < 614400; i++)
		{
			sprintf(tvin_gmem_offset, "%s\n", (gmem_offset_tvin_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_gmem_offset, tvin_gmem_offset);
		}

		tcl_file.set_num(614400, &tcl_file.gmem_offset_depth);
		sprintf(tvin_gmem_offset, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_gmem_offset, tvin_gmem_offset);

		// release memory allocation
		delete [] gmem_offset_tvin_wrapc_buffer;

		// [[transaction]]
		sprintf(tvin_frame_in, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_frame_in, tvin_frame_in);

		sc_bv<32> frame_in_tvin_wrapc_buffer;

		// RTL Name: frame_in
		{
		}

		// dump tv to file
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_frame_in, "%s\n", (frame_in_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_frame_in, tvin_frame_in);
		}

		tcl_file.set_num(1, &tcl_file.frame_in_depth);
		sprintf(tvin_frame_in, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_frame_in, tvin_frame_in);

		// [[transaction]]
		sprintf(tvin_frame_out, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_frame_out, tvin_frame_out);

		sc_bv<32> frame_out_tvin_wrapc_buffer;

		// RTL Name: frame_out
		{
		}

		// dump tv to file
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_frame_out, "%s\n", (frame_out_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_frame_out, tvin_frame_out);
		}

		tcl_file.set_num(1, &tcl_file.frame_out_depth);
		sprintf(tvin_frame_out, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_frame_out, tvin_frame_out);

		// [[transaction]]
		sprintf(tvin_init, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_init, tvin_init);

		sc_bv<1> init_tvin_wrapc_buffer;

		// RTL Name: init
		{
			// bitslice(0, 0)
			{
				// celement: init(0, 0)
				{
					// carray: (0) => (0) @ (0)
					{
						// sub                   : 
						// ori_name              : init
						// sub_1st_elem          : 
						// ori_name_1st_elem     : init
						// regulate_c_name       : init
						// input_type_conversion : init
						if (&(init) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<1> init_tmp_mem;
							init_tmp_mem = init;
							init_tvin_wrapc_buffer.range(0, 0) = init_tmp_mem.range(0, 0);
						}
					}
				}
			}
		}

		// dump tv to file
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_init, "%s\n", (init_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_init, tvin_init);
		}

		tcl_file.set_num(1, &tcl_file.init_depth);
		sprintf(tvin_init, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_init, tvin_init);

		// [[transaction]]
		sprintf(tvin_bgmodel, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_bgmodel, tvin_bgmodel);

		sc_bv<32> bgmodel_tvin_wrapc_buffer;

		// RTL Name: bgmodel
		{
		}

		// dump tv to file
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_bgmodel, "%s\n", (bgmodel_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_bgmodel, tvin_bgmodel);
		}

		tcl_file.set_num(1, &tcl_file.bgmodel_depth);
		sprintf(tvin_bgmodel, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_bgmodel, tvin_bgmodel);

// [call_c_dut] ---------->

		AESL_ORIG_DUT_bgsub(frame_in, frame_out, init, bgmodel);


		// [[transaction]]
		sprintf(tvout_gmem, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVOUT_gmem, tvout_gmem);

		sc_bv<8>* gmem_tvout_wrapc_buffer = new sc_bv<8>[153600];

		// RTL Name: gmem
		{
			// bitslice(7, 0)
			{
				int hls_map_index = 0;
				// celement: frame_in(7, 0)
				{
					// carray: (0) => (76799) @ (1)
					for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : frame_in[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : frame_in[0]
						// regulate_c_name       : frame_in
						// input_type_conversion : frame_in[i_0]
						if (&(frame_in[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<8> frame_in_tmp_mem;
							frame_in_tmp_mem = frame_in[i_0];
							gmem_tvout_wrapc_buffer[hls_map_index++].range(7, 0) = frame_in_tmp_mem.range(7, 0);
						}
					}
				}
				// celement: frame_out(7, 0)
				{
					// carray: (0) => (76799) @ (1)
					for (int i_0 = 0; i_0 <= 76799; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : frame_out[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : frame_out[0]
						// regulate_c_name       : frame_out
						// input_type_conversion : frame_out[i_0]
						if (&(frame_out[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<8> frame_out_tmp_mem;
							frame_out_tmp_mem = frame_out[i_0];
							gmem_tvout_wrapc_buffer[hls_map_index++].range(7, 0) = frame_out_tmp_mem.range(7, 0);
						}
					}
				}
			}
		}

		// dump tv to file
		for (int i = 0; i < 153600; i++)
		{
			sprintf(tvout_gmem, "%s\n", (gmem_tvout_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVOUT_gmem, tvout_gmem);
		}

		tcl_file.set_num(153600, &tcl_file.gmem_depth);
		sprintf(tvout_gmem, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVOUT_gmem, tvout_gmem);

		// release memory allocation
		delete [] gmem_tvout_wrapc_buffer;

		// [[transaction]]
		sprintf(tvout_gmem_offset, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVOUT_gmem_offset, tvout_gmem_offset);

		sc_bv<32>* gmem_offset_tvout_wrapc_buffer = new sc_bv<32>[614400];

		// RTL Name: gmem_offset
		{
			// bitslice(31, 0)
			{
				int hls_map_index = 0;
				// celement: bgmodel(31, 0)
				{
					// carray: (0) => (614399) @ (1)
					for (int i_0 = 0; i_0 <= 614399; i_0 += 1)
					{
						// sub                   : i_0
						// ori_name              : bgmodel[i_0]
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : bgmodel[0]
						// regulate_c_name       : bgmodel
						// input_type_conversion : *(int*)&bgmodel[i_0]
						if (&(bgmodel[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> bgmodel_tmp_mem;
							bgmodel_tmp_mem = *(int*)&bgmodel[i_0];
							gmem_offset_tvout_wrapc_buffer[hls_map_index++].range(31, 0) = bgmodel_tmp_mem.range(31, 0);
						}
					}
				}
			}
		}

		// dump tv to file
		for (int i = 0; i < 614400; i++)
		{
			sprintf(tvout_gmem_offset, "%s\n", (gmem_offset_tvout_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVOUT_gmem_offset, tvout_gmem_offset);
		}

		tcl_file.set_num(614400, &tcl_file.gmem_offset_depth);
		sprintf(tvout_gmem_offset, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVOUT_gmem_offset, tvout_gmem_offset);

		// release memory allocation
		delete [] gmem_offset_tvout_wrapc_buffer;

		// release memory allocation: "gmem"
		delete [] tvin_gmem;
		delete [] tvout_gmem;
		// release memory allocation: "gmem_offset"
		delete [] tvin_gmem_offset;
		delete [] tvout_gmem_offset;
		// release memory allocation: "frame_in"
		delete [] tvin_frame_in;
		// release memory allocation: "frame_out"
		delete [] tvin_frame_out;
		// release memory allocation: "init"
		delete [] tvin_init;
		// release memory allocation: "bgmodel"
		delete [] tvin_bgmodel;

		AESL_transaction++;

		tcl_file.set_num(AESL_transaction , &tcl_file.trans_num);
	}
}

