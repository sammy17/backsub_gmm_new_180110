...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.bgsub.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.bgsub.autotvout_gmem.dat"
...
output [32 - 1 : 0] TRAN_gmem_frame_in;
output [32 - 1 : 0] TRAN_gmem_frame_out;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] frame_in = 0;
reg [32 - 1 : 0] frame_out = 0;
...
assign    TRAN_gmem_frame_in = frame_in;
assign    TRAN_gmem_frame_out = frame_out;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
